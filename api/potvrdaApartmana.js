...
module.exports = async (req, res) => {
  try {
    ...
    if (
      ...
      !guests || !calculated_price ||
      !first_name || !last_name || !email || !phone
    ) {
      return res.status(400).json({
        message: "Nedostaju podaci za rezervaciju."
      });
    }
    ...
    return res.status(200).json({
      message: `✅ Rezervacija za *${selected.name}* od ${checkin_date} do ${checkout_date} za ${guests} osobe je uspešno zabeležena!\nUkupna cena: ${calculated_price} €.\n\n📧 Uskoro ćemo kontaktirati ${first_name} na ${email} ili ${phone}. Hvala vam! 😊`,
    ...
    if (error.response) {
      ...
      console.error("Greška pri rezervaciji - nema odgovora:", error.request);
    } else {
      console.error("Greška pri rezervaciji - poruka:", error.message);
    }

    return res.status(500).json({
      message: "Greška pri slanju rezervacije ka OTA Sync sistemu. Pokušajte ponovo kasnije."
    });
  }
};
